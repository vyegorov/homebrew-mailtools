class Mb2md < Formula
  ...
  def install
    ## The following patches are applied to the downloaded source.
    ## Keep them around in case further modifications are required, as
    ## Formulae requires a total patch in a single bundle.
    # sed -e '1,/^$/d' patches/mb2md-01-fix-dash-r-option.patch | patch -p1 mb2md-3.20.pl
    # sed -e '1,/^$/d' patches/mb2md-02-better-separator-line-detection.patch | patch -p1 mb2md-3.20.pl
    # sed -e '1,/^$/d' patches/mb2md-03-fix-uw-imap-misspellings.patch | patch -p1 mb2md-3.20.pl
    # sed -e '1,/^$/d' patches/mb2md-04-use-HOME-to-determine-home-directory.patch | patch -p1 mb2md-3.20.pl
    # sed -e '1,/^$/d' patches/mb2md-05-Make-source-and-destination-path-expansion-behave-mo.patch | patch -p1 mb2md-3.20.pl
...
+#                Use with caution!  Results may be unreliable.  I recommend to
+#                do a run without "-c" first and only use it if you are certain,
...
+#                /var/spool/mail/blah for user blah and the destination mailbox
+#                will be the "destdir" mailbox itself.
 #
...
+#                mailboxes' names.  See the examples below.
+#
...
+#                subfolders from the IMAP user's point of view.
...
+#                the path is considered to be absolute, otherwise it is
+#                relative to the users home directory.
+#
+#  -r strip_ext  If defined this extension will be stripped from the original
+#                mailbox file name before creating the corresponding maildir.
+#                The extension must be given without the leading dot (".").
+#                See the example below.
+#
+#  -l UW-file    File containing the list of subscribed folders.  If migrating
+#                from UW-IMAP the list of subscribed folders will be found in
...
+#                for i in *; do
+#                  echo $i
+#                  su - $i -c "/bin/mb2md -s mail -l .mailboxlist -R -d Maildir"
+#                done
 #
 #
 #  Example
...
+# With the UW IMAP server, fffff and ggggg would have appeared in the root of
+# this mail server, along with the Inbox.  aaaa, bbbb etc, would have appeared
+# in a folder called xxx from that root, and xxx was just a folder not a
...
-# usage.
-#
+# "IMAP Folders" button and use "Reset list".  In the "All" window, select the
...
+# so they appear in an IMAP folder:
 #
 #    Inbox -----------------
 #        | music
@@ -347,35 +343,33 @@
...
-# hierarchy:
+# If you don't strip the extension "mbx" you will get the following hierarchy:
 #
 # Inbox
 #      |
-#       - Trash 
...
+usage() if ( defined($opts{h}) || ( !defined($opts{m}) && !defined($opts{s}) ) );
...
-if (defined($opts{m}))
+# First, if the user has gone with the -m option we simply convert their mailfile
+if ( defined($opts{m}) )
 {
-	if (defined($ENV{'MAIL'})) {
+	if ( defined($ENV{'MAIL'}) ) {
...
+# Count the number of mailboxes, or at least files, we found.
 my $mailboxcount = 0;
 
-# Since we'll be making sub directories of the main
-# Maildir, we need to make sure that the main maildir
-# exists
+# Since we'll be making sub directories of the main Maildir,
+# we need to make sure that the main maildir exists
 &maildirmake($dest);
...
-# for each mbox file.
+# This function creates all subdirs in maildir, and calls convert() for each mbox file.
...
+		# if $strip_extension is defined, strip it off the $targetfile
+		#		defined($strip_ext) && ( $destinationdir =~ s/\_$strip_ext$// );
...
+	# if it is the root maildir (i.e. converting the inbox)
...
+	my $fromline = '';
+
+	# Set to 1 when we are reading the header lines, including the "From " line.
+	#
+	# 0 means we are reading the message body and looking for next "From " line.
+	my $inheaders = 0;
+
+	# Variable to hold all headers apart from the first line "From ...."
+	# which is not part of the message itself.
+	my $headers = '';
+
+	# Variable to hold the accumulated characters we find in header lines
+	# of the type:
+	#
+	#    Status:
+	#    X-Status:
...
+	my $messagefn = '';
+
+	# The date string from the "From " line of each message will be written here
+	# -- and used by touch to alter the date-time of each message file.  Put
+	# non-date text here to make it spit the dummy if my code fails to find a
+	# date to write into this.
+	my $receivedate = 'Bogus';
...
-		if ($use_cl eq 1)
+	# We record the message start line here, for error reporting.
+	my $startline;
+
+	# If defined, we use this as the number of bytes in the
+	# message body rather than looking for a /^From / line.
+	my $contentlength;
+
+	# A From lines can either occur as the first line of a file, or after an
+	# empty line.  Most mail systems will quote all From lines appearing in the
+	# message, but some will only do it when necessary.  Since we initialise
+	# the variable to true, we don't need to check for beginning of file.
+
+	while ( <MBOX> )
+	{
+		# exchange possible Windows EOL (CRLF) with Unix EOL (LF)
...
+			$startline = $.;
+
+			# If this is not the first run through the loop then this means we
+			# have already been working on a message.
+			if ( $messagecount > 0 )
+			{
+				# If so, then close that message file and then use utime to
+				# change its date-time.
+				#
+				# Note this code should be duplicated to do the same thing at
+				# the end of the while loop since we must close and touch the
+				# final message file we were writing when we hit the end of the
+				# Mbox file.
+
+				close (OUT);
+
...
+			# IMAP server encoded body lines starting with "From ".
+			#
+			# Pattern matching Perl majik to get rid of an Mbox quoted From.
+			#
+			# This works on the default variable "$_" which contains the text
+			# from the Mbox mailbox - I guess this is the case because of our
+			# (open(MBOX ....) line above, which did not assign this to
+			# anything else, so it would go to the default variable.  This
+			# enables inscrutably terse Perlisms to follow.
+			#
+			# "s" means "Subsitute" and it looks for any occurrence of ">From"
+			# starting at the start of the line.  When it finds this, it
+			# replaces it with "From".
+			#
+			# So this finds all instances in the Mbox message where the
+			# original line started with the word "From" but was converted to
+			# ">From" in order to not be mistaken for the "From ..." line which
+			# is used to demark each message in the Mbox.  This was was a
+			# destructive conversion because any message which originally had
+			# ">From" at the start of the line, before being put into the Mbox,
+			# will now have that line without the ">".
+			s/^>From /From /;
+
+			# Glorious tersness here.  Thanks Simon for explaining this.
+			#
+			# "print OUT" means print the default variable to the file of file
+			# handle OUT.  This is where the bulk of the message text is
...
-            $messagecount--;
+	# Close the output file, and duplicate the code from the start of the while
+	# loop which touches the date-time of the most recent message file.
...
+	}
+
+	# After all the messages have been converted, check to see if the first one
+	# was a dummy.  if so, delete it and make the message count one less.
+	if ( $deletedummy ne "" )
+	{
+		printf("Dummy mail system first message detected and not saved.\n");
+		unlink $deletedummy;
